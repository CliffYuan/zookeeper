...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("Processing request:: {},", request, (request.hdr != null ? "进行zkDataTree操作" : "不进行数据更新操作"));
        }
        ...
        synchronized (zks.outstandingChanges) {
            ...
            if (request.hdr != null) {
                ...
                rc = zks.processTxn(hdr, txn);//todo 更新内存结构数据,触发watch
            }
        ...
        if (request.cnxn == null) {
            LOG.info("请求业务操作完成，不发送响应,request{}", request);
            return;
        } else {
            LOG.info("请求业务操作完成，发送响应给客户端,RquestIp:{},request{}", request.cnxn.getRemoteSocketAddress(), request);
        }
